...
const r=require;
const root=path.resolve(__dirname,'..'),kit=path.join(root,'deliverables/numer-brand-kit-v1.0-rc2'),out=path.join(root,'assets/unified-wordmark');
(async()=>{
 ...
 const mark=fs.readFileSync(path.join(kit,'wordmark/numer-wordmark-petroleum.svg')).toString('base64');
 await page.setContent('<style>html,body{margin:0;background:#F5F6F7}body{display:flex;align-items:center;justify-content:center;width:960px;height:320px}img{width:630px;height:auto}</style><img alt="Numer con la n del icono integrada" src="data:image/svg+xml;base64,'+mark+'">');
 await page.locator('img').evaluate(i=>i.decode());await page.screenshot({path:path.join(kit,'numer-wordmark-preview.png')});
