...
KIT=ROOT/'deliverables/numer-brand-kit-v1.0-rc2'
OUT=ROOT/'assets/unified-wordmark'
...
pdf=KIT/'guide/numer-brand-manual-v1.0-rc2.pdf'
...
def img(name,x,y,w):
    h=w*(70/333 if name.startswith('numer-wordmark') else 1)
...
def base(n,section):
    ...
    label('1.0 RC2 · Para revisión · 15.09.2026',44,H-33);label(f'{n:02d} / 07',500,H-33)
...
para('El resto de la entrega cuenta con tu visto bueno. Esta revisión integra la n del icono en el nombre completo, como único cambio solicitado.',64,607,450,10,'#D7E7E3')
label('MANUAL DE MARCA 1.0 RC2',44,739,PAPER);label('15.09.2026 / 01',437,739,PAPER)
nextpage()

# 2: assets and variants
base(2,'IDENTIDAD');heading('01 / FIRMA','Una identidad, dos piezas.','La n del icono forma ahora parte del nombre completo. Conserva su contorno continuo; las letras umer mantienen su dibujo anterior.')
img('numer-wordmark-petroleum',57,239,290);img('numer-icon',438,221,86)
label('NOMBRE / INTEGRACIÓN BR-C31',57,328);label('ICONO / BR-A05',424,328)
para('Usa el nombre completo en presentación y acceso. Usa el icono en favicon y espacios pequeños. La misma n personalizada une ambas aplicaciones.',44,370,500,11)
label('VARIANTES DE TINTA / CRITERIO ACEPTADO',44,458,P)
...
para('SVG con contornos, sin dependencia de fuentes. La n conserva el trazo del símbolo y umer conserva su dibujo. Las tres variantes comparten la misma geometría.',44,685,500,11,MUTED)
nextpage()

# 3: clearspace and sizes
base(3,'USO DEL LOGO');heading('02 / ESPACIO Y ESCALA','Dale espacio para respirar.','Reglas aceptadas y lectura revisada con la inicial integrada. El tamaño mínimo es una referencia de pantalla, no una certificación para todos los soportes.')
img('numer-wordmark-petroleum',107,262,333)
c.setStrokeColor(HexColor(P));c.setDash(3,3)
# Visible bounds are x=6.95, y=16.5, w=309.25, h=57 in viewBox 0 9 333 70.
c.rect(107+6.95-28.5,H-(262+7.5+57+28.5),309.25+57,114,stroke=1,fill=0);c.setDash()
...
para('Revisado en Chrome a escala 1x: la nueva composición se lee a 80 px; se mantiene 96 px como referencia habitual. Por debajo de 80 px, prefiere el icono.',44,744,505,9,MUTED,leading=13)
nextpage()
...
# 5 voice
base(5,'VOZ');heading('04 / VOZ ACEPTADA','Clara. Calmada. Cercana.','Habla de tú, explica el siguiente paso y describe hechos. Numer acompaña sin juzgar cómo una persona gasta, ahorra o gestiona sus deudas.')
...
label('MOVIMIENTO DE MARCA / DIRECCIÓN ACEPTADA',44,613,P)
...
base(7,'CIERRE Y ENTREGA');heading('06 / REVISIÓN FINAL','Una misma n para Numer.','Tu visto bueno conserva el resto de la entrega. Esta revisión cambia únicamente la inicial del nombre y las referencias necesarias para usarlo.')
sections=[('BASE ACEPTADA','Nombre Numer, monograma e icono; mensaje, voz, colores, tipografía y reglas de uso de la entrega anterior. La maqueta de interfaz conserva sus pendientes.'),('ÚNICO CAMBIO SOLICITADO','La n del icono sustituye a la n con corte del nombre completo. Se escala de forma uniforme y se ajusta la separación; umer conserva sus contornos. Esta es la nueva composición para valoración visual.'),('DESPUÉS, CON UX/UI','Radios y altura del estado activo del sidebar; componentes, recorridos, estados, contraste y adaptación. Producto y tecnología definen reglas financieras y seguridad.'),('ARCHIVOS INCLUIDOS','SVG de logo y símbolo, icono SVG/PNG, favicon ICO, fuentes y licencia, manual PDF, guías editables, handoff e inventario de integridad.')]
y=230
