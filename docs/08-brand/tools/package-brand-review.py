...
reader=PdfReader(kit/'guide/numer-brand-manual-v1.0-rc1.pdf')
assert len(reader.pages)==7
texts=[p.extract_text() for p in reader.pages]
for i,needle in enumerate(['Tus cuentas y tarjetas','Una identidad','Dale espacio','Petróleo mineral','Clara. Calmada. Cercana.','La confianza','Listo para decidir']):assert needle in texts[i],(i,needle)
embedded=[]
...
verification={'edition':'1.0-rc1','status':'produced and locally verified; creative approval pending','date':'2026-09-15','original_assets_byte_identical':preserved,'font_files_byte_identical':True,'variant_alpha_identical':True,'svg_xml_valid':True,'local_document_links_valid':True,'pdf_pages':7,'pdf_text_checked':True,'pdf_embedded_fonts':sorted(set(embedded)),'pdf_visual_review':'All seven final rendered pages inspected; no clipping or overlap observed.','size_review':json.loads((root/'assets/brand-review/render-verification.json').read_text()),'png_dimensions':pngs,'ico_sizes':sorted(ico.ico.sizes()),'limitations':['No print validation','No full accessibility audit','No commercial name clearance','No app or security implementation']}
...
(kit/'manifest.json').write_text(json.dumps({'edition':'1.0-rc1','files':entries},indent=2,ensure_ascii=False)+'\n')
