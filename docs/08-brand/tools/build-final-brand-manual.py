...
pdf=KIT/'guide/numer-brand-manual-v1.0.pdf'
c=canvas.Canvas(str(pdf),pagesize=(W,H));c.setTitle('Numer | Manual de marca 1.0');c.setAuthor('Numer / Brand strategy')
boxes=[]
...
def base(n,section):
    ...
    label('1.0 · Identidad elegida · 15.09.2026',44,H-33);label(f'{n:02d} / 07',500,H-33)
...
# 1: cover and status
rect(0,0,W,H,P)
label('IDENTIDAD / EDICIÓN 1.0',44,36,PAPER)
...
para('Esta edición conserva el nombre de peso uniforme con corte diagonal y la n personalizada como símbolo independiente. Voz, colores y reglas mantienen tu visto bueno.',64,607,450,10,'#D7E7E3')
label('MANUAL DE MARCA 1.0',44,739,PAPER);label('15.09.2026 / 01',437,739,PAPER)
nextpage()
...
label('VARIANTES DE TINTA / ACEPTADAS',44,458,P)
...
base(3,'USO DEL LOGO');heading('02 / ESPACIO Y ESCALA','Dale espacio para respirar.','Reglas aceptadas y comprobación local de lectura. El tamaño mínimo es una referencia de pantalla, no una certificación para todos los soportes.')
...
base(7,'CIERRE Y ENTREGA');heading('06 / ENTREGA DE MARCA','La identidad de Numer, reunida.','El nombre original y el icono cumplen funciones distintas. Esta edición reúne la identidad elegida; la maqueta y los componentes continúan con UX/UI.')
sections=[('YA ELEGIDO','Nombre Numer, dibujo del logotipo, monograma e icono. Dirección Petróleo mineral y preferencia por Manrope. Base visual del resumen aceptada, excepto el sidebar.'),('CRITERIOS ACEPTADOS','Mensaje principal y descriptor; voz; tintas; área de protección; mínimo de 80 px y 96 px habitual; usos del logo, pesos de marca y dirección de movimiento. La n integrada se descarta por desequilibrio de peso.'),('DESPUÉS, CON UX/UI','Radios y altura del estado activo del sidebar; componentes, recorridos, estados, contraste y adaptación. Producto y tecnología definen reglas financieras y seguridad.'),('ARCHIVOS INCLUIDOS','SVG de logo y símbolo, icono SVG/PNG, favicon ICO, fuentes y licencia, manual PDF, guías editables, handoff e inventario de integridad.')]
y=230
