...
preserved=[]
for folder in ['symbol','icon','favicon']:
...
original=ET.parse(old/'wordmark/numer-wordmark-petroleum.svg').find('.//s:g/s:path',ns).attrib['d']
expected='M86.350'+original.split(' M86.350',1)[1]
monogram=ET.parse(old/'symbol/numer-symbol-petroleum.svg').find('.//s:path',ns).attrib['d']
for p in kit.rglob('*.svg'):
 tree=ET.parse(p)
 if p.parent.name=='wordmark':
  paths=tree.findall('.//s:path',ns)
  assert paths[0].attrib['d']==monogram
  assert paths[1].attrib['d']==expected
  assert tree.getroot().attrib['viewBox']=='0 9 333 70'
...
reader=PdfReader(kit/'guide/numer-brand-manual-v1.0-rc2.pdf')
assert len(reader.pages)==7
texts=[p.extract_text() for p in reader.pages]
for i,needle in enumerate(['Tus cuentas y tarjetas','Una identidad','Dale espacio','Petróleo mineral','Clara. Calmada. Cercana.','La confianza','Una misma n']):assert needle in texts[i],(i,needle)
embedded=[]
...
verification={'edition':'1.0-rc2','status':'BR-A07 accepts remaining brand kit; new initial composition delivered for visual review','date':'2026-09-15','original_assets_byte_identical':preserved,'monogram_path_identical':True,'umer_path_identical':True,'font_files_byte_identical':True,'variant_alpha_identical':True,'svg_xml_valid':True,'local_document_links_valid':True,'pdf_pages':7,'pdf_text_checked':True,'pdf_embedded_fonts':sorted(set(embedded)),'pdf_visual_review':'All seven final rendered pages inspected; no clipping or overlap observed.','size_review':json.loads((root/'assets/unified-wordmark/render-verification.json').read_text()),'png_dimensions':pngs,'ico_sizes':sorted(ico.ico.sizes()),'limitations':['No print validation','No full accessibility audit','No commercial name clearance','No app or security implementation']}
...
(kit/'manifest.json').write_text(json.dumps({'edition':'1.0-rc2','files':entries},indent=2,ensure_ascii=False)+'\n')
